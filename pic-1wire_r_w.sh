...
size_check() {
  local result i
  for ((i = 0; i < 3; i++)); do
    result=$(sudo ./idll-test.exe --dallas-eeprom-write 0:0:255 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_write | grep -i "device $i" | sed "s/1Wire - device $i size: //g" | sed "s/\s//g")
...
WireWriteSave_Auto() {
  ...
  while true; do
    ...
    if [[ "$input" == "q" || "$input" == "" ]]; then
      ...
      launch_command "sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Same_Pattern_0xA5"
      compare_result "$result" "pass"
      launch_command "sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Random_Pattern"
      compare_result "$result" "pass"
    fi
...
#    echo "choise=$choise"
#    echo "data=$data"
    launch_command "sudo ./idll-test.exe --dallas-eeprom-write $choise:$position:$data -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_write"
...
#  fi
  ...
  launch_command "sudo ./idll-test.exe --dallas-eeprom-read $choise:$position:1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_read"
...
SearchID() {
  ...
  print_command "sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_IDs"
...
BadParameter() {
  ...
  for all in $(seq 0 2); do
    printf "${COLOR_RED_WD}sudo ./idll-test.exe --dallas-eeprom-write $all:$bad:255/255/255 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_write ${COLOR_REST}\n"
    sudo ./idll-test.exe --dallas-eeprom-write $all:$bad:255/255/255 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_write
    sleep 0.5
    printf "${COLOR_RED_WD}sudo ./idll-test.exe --dallas-eeprom-read $all:$bad:3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_read ${COLOR_REST}\n"
    sudo ./idll-test.exe --dallas-eeprom-read $all:$bad:3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_read
  done
  ...
  bad="g/g/g"
  for all in $(seq 0 2); do
    printf "${COLOR_RED_WD}sudo ./idll-test.exe --dallas-eeprom-write $all:0:$bad -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_write ${COLOR_REST}\n"
    sudo ./idll-test.exe --dallas-eeprom-write $all:0:$bad -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_write
  done
  ...
  read -p "" continue
  for all in $(seq 0 2); do
    printf "${COLOR_RED_WD}sudo ./idll-test.exe --dallas-eeprom-read $all:0:-1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_read ${COLOR_REST}\n"
    sudo ./idll-test.exe --dallas-eeprom-read $all:0:-1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_read
  done
}
...
write() {
  ...
  launch_command "sudo ./idll-test.exe --dallas-eeprom-write $2:0:$1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_write"
...
#  data_length=$(($1 + 1))
  ...
  launch_command "sudo ./idll-test.exe --dallas-eeprom-read $2:0:$data_length -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_read"
