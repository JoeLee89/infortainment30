...
GetSetPin_bsec(){
  ...
  for (( i = 24; i < 32; i++ )); do
    if [ "$input" == "q" ]; then
      ...
      sudo ./idll-test.exe --DO_PIN_NUM $i --MSENSE_PIN_NUM $m   -- --EBOARD_TYPE EBOARD_ADi_SC1X --section HardMeter_BACC_SetPin
      (( m++ ))
done
}
...
GetSetPort_bsec(){
  ...
  for i in 1 2 4 8 16 32 64 128 255; do
    if [ "$input" == "q" ]; then
      ...
      sudo ./idll-test.exe --PORT_VAL $i -- --EBOARD_TYPE EBOARD_ADi_SC1X --section HardMeter_BACC_SetPort
  done
}
...
GetSetPin_SA3(){
  ...
  for (( i = 0; i < 16; i++ )); do
    ...
    launch_command "sudo ./idll-test.exe --PIN_VAL $i --HM-Int-Count 1 -- --EBOARD_TYPE EBOARD_ADi_SC1X --section HardMeter_ByPin"
...
SetGetPort_SA3(){
  ...
  while true; do
    launch_command "sudo ./idll-test.exe --PORT_VAL $hex --HM-Int-Count 1 -- --EBOARD_TYPE EBOARD_ADi_SC1X --section HardMeter_ByPort"
    compare_result "$result" "passed"
    hex=$((hex<<1))
    ...
    case $hex in
      ...
      $((2#10000000000000000)))
        ...
        read -p "enter key to test..." continue
          ...
          while true; do
            now=$(date +%s)
            launch_command "sudo ./idll-test.exe --PORT_VAL 65535 --HM-Int-Count 1 -- --EBOARD_TYPE EBOARD_ADi_SC1X --section HardMeter_ByPort"
            compare_result "$result" "passed"
            if [[ "$now" > $after ]]; then
              break
            fi
...
SetGetPort_SCXX(){
  ...
  while true; do
      now=$(date +%s)
      launch_command "sudo ./idll-test.exe --PORT_VAL 255 --HM-Int-Count 1 -- --EBOARD_TYPE EBOARD_ADi_SC1X --section HardMeter_ByPort"
      compare_result "$result" "passed"
      if [[ "$now" > $after ]]; then
        break
      fi
    ...
    "plug")
      while true; do
        ...
        for (( l = 0; l < $meter_total_pin; l++ )); do
          ...
          launch_command "sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SC1X --section HardMeter_Detection_ByPort"
          compare_result "$result" "$expected_getport_plug_value"

          printf "\n\n"
          title b "Now get detection ( PIN ) value"
          launch_command "sudo ./idll-test.exe --HM_PIN_ID $l -- --EBOARD_TYPE EBOARD_ADi_SC1X --section HardMeter_Detection_ByPin"
...
#increment_port(){
#
##  start /B /wait idll-test --PORT_VAL 0x1FF --HM-Int-Count 1 -- --EBOARD_TYPE EBOARD_ADi_SC1X --section HardMeter_ByPort
...
#      m=$((2**$i))
#      l=$((m+l))
#      launch_command "sudo ./idll-test.exe --PORT_VAL $l --HM-Int-Count $increment_number -- --EBOARD_TYPE EBOARD_ADi_SC1X --section HardMeter_ByPort"
