...
#printf "data=$data \n"
#printf "len=$len \n"
#sudo ./idll-test.exe --sram-write 1:0:$data -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write

#sudo ./idll-test.exe --sram-write 1:100:2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2/2 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write


#while true; do
#  for i in {0..16777215..10}; do
#    sudo ./idll-test.exe --sram-write 1:$i:255/255/255/255/255/255/255/255/255/255 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write
##    sudo ./idll-test.exe --sram-read 1:100:3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read
#  done
#done
...
sram_info(){
  ...
  bank=$(sudo ./idll-test.exe --SOURCE_BANK 0x0 --DEST_BANK 0x1 --ADDRESS 0x0 --LENGTH 0x1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramBankCompareManual | grep -i "sram bank" | sed 's/SRAM Bank\[Number:Size\]\=\[0x//g' | sed 's/:0x[0-9]*]//g' | sed 's/\s//g')
  #display each bank capacity in hex unit
  bank_capacity_hex=$(sudo ./idll-test.exe --SOURCE_BANK 0x0 --DEST_BANK 0x1 --ADDRESS 0x0 --LENGTH 0x1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramBankCompareManual | grep -i "sram bank" | sed 's/SRAM Bank\[Number:Size\]\=\[0x//g' | sed 's/[0-9]:0x//g' | sed 's/\]//g' | sed 's/\s//g')
#  echo "bankhex=$bank_capacity_hex"
#  #display each bank capacity in hex unit
#  bank_capacity_hex=$(echo ${bank_capacity_hex:0:6})

#  bank_amount=$(sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH 0x800000 --SOURCE_BANK 0x0 --DEST_BANK 0x1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramBankCopyManual | grep -i "sram bank" | sed 's/SRAM Bank\[Number:Size\]\=\[0x//g')
  bank_amount=$(sudo ./idll-test.exe --SOURCE_BANK 0x0 --DEST_BANK 0x1 --ADDRESS 0x0 --LENGTH 0x1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramBankCompareManual | grep -i "sram bank" | sed 's/SRAM Bank\[Number:Size\]\=\[0x//g' | sed 's/\s//g')
...
#  address=$( sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Capacity | grep -i 'size' | sed 's/SRAM size: //g' | sed 's/\/r//g')
  address=$(sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Capacity | grep -i 'size' | sed 's/SRAM size: //g' | sed 's/\/r//g' | sed 's/\s//g' |sed 's/]//g')
...
#  echo "bankamount=$bank_amount"
  for (( p = 0; p < bank_amount; p++ )); do
    ...
    bank_address_list[$p]=$((address_dec*p))
  done
...
#config list
#===============================================================
#input write how many byte you need 1=1byte=11111111 bit
#loop=16777216
sram_info
write_data
...
SramSyncVsync_Repeat(){
  ...
  for (( i = 0; i < 10; i++ )); do
    ...
    launch_command "sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SA3X --section ASYNC_SRAM"
    verify_result "$result"

    title b "Sync sram test (repeat 10 times)"
    launch_command "sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SYNC_SRAM"
    verify_result "$result"
...
SramAutoRandomSame(){
  ...
  for (( i = 0; i < 10; i++ )); do
    ...
    launch_command "sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Same_Pattern_0xA5"
    verify_result "$result"

    launch_command "sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Random_Pattern"
    verify_result "$result"
...
SramCapacity(){
  ...
  launch_command "sudo ./idll-test.exe -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Capacity"
  sram_info
...
#  write_data=("255" "254" "111" "99" "0")
  ...
  for f in 0 1; do
    ...
    for (( i = 0; i < loop; i++ )); do
        ...
        if [ $f == 0 ]; then
          launch_command "sudo ./idll-test.exe --sram-write 1:$i:${write_data[$m]} -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write"
        else
          launch_command "sudo ./idll-test.exe --sram-write 1:$i:255 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write"
        fi
    ...
    #sudo ./idll-test.exe --dallas-eeprom-write 0:4:99 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section PIC_1Wire_EEPROM_Manual_write
    ...
    for (( i = 0; i < loop; i++ )); do
        ...
        launch_command "sudo ./idll-test.exe --sram-read 1:$i:1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read"
...
write_wrong_address(){
  ...
  launch_command "sudo ./idll-test.exe --sram-read $1:$address:1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read"
  compare_result "$result" "failed"

  echo ""
  title b "Now write the address out of the define in mode$1 : $address"
  launch_command "sudo ./idll-test.exe --sram-write $1:$address:255 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write"
  compare_result "$result" "failed"
}
...
Sram_Mirror_1_All(){
  ...
  read -p "" input_size
  loop=${input_size:-sram_each_bank_size1}
  ...
  for (( i = 0; i < loop; i++ )); do
      ...
      launch_command "sudo ./idll-test.exe --sram-write $mirror_mode:$i:${write_data[$m]} -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write"
  ...
  for (( i = 0; i < loop; i++ )); do
      ...
      launch_command "sudo ./idll-test.exe --sram-read $mirror_mode:$i:1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read"
      compare_result "$result" "$data"
      ...
      for address in ${bank_address[*]}; do
        if [ "$address" != "0" ]; then
          ...
          launch_command "sudo ./idll-test.exe --sram-read 1:$(( address+i )):1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read"
          compare_result "$result" "$data"
        fi
...
Sram_Mirror_2_2_ScxxSa3(){
  ...
  for (( i = 0; i < loop; i++ )); do
      ...
      launch_command "sudo ./idll-test.exe --sram-write $mirror22_mode:$i:${write_data[$m]} -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write"

      #write data in bank1 , the address will refer to bank1 first address + $i parameter
      launch_command "sudo ./idll-test.exe --sram-write $mirror22_mode:$((i+bank_address[1])):${write_data[$m]} -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write"
  ...
  for (( i = 0; i < loop; i++ )); do
      ...
      launch_command "sudo ./idll-test.exe --sram-read $mirror22_mode:$i:1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read"
      verify_result "$result"
      compare_result "$result" "$data"

      launch_command "sudo ./idll-test.exe --sram-read $mirror22_mode:$((i+bank_address[1])):1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read"
      verify_result "$result"
      compare_result "$result" "$data"
      ...
      for a in ${bank_address[0]} ${bank_address[2]}; do
        if [ "$a" ]; then
          launch_command "sudo ./idll-test.exe --sram-read 1:$((i+a)):1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read"
          verify_result "$result"
          compare_result "$result" "$data"
        fi
...
#===============================================================
# (LEC1) SRAM mirror 2 to 2 function test
#===============================================================
Sram_Mirror_2_2_Lec1(){
  ...
  for (( i = 0; i < loop; i++ )); do
      ...
      if [ ${bank_address[2]} ]; then
        launch_command "sudo ./idll-test.exe --sram-write $mirror_mode:$((i+bank_address[2])):${write_data[$m]} -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write"
        verify_result "$result"
  ...
  for (( i = 0; i < loop; i++ )); do
      ...
      launch_command "sudo ./idll-test.exe --sram-read $mirror_mode:$i:1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read"
      verify_result "$result"
#      result=$( sudo ./idll-test.exe --sram-read $mirror_mode:$i:1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read )
#      printf  "$result\n"
      compare_result "$result" "$data"

      #bank2 data compare with mirror2
      if [ ${bank_address[2]} ]; then
        launch_command "sudo ./idll-test.exe --sram-read $mirror_mode:$((i+bank_address[2])):1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read"
        verify_result "$result"
        compare_result "$result" "$data"
      fi
...
BadParameter(){
  ...
  printf  "${COLOR_RED_WD}sudo ./idll-test.exe --sram-write 1:100000000000:255/255/255 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write  ${COLOR_REST}\n"
  sudo ./idll-test.exe --sram-write 1:100000000000:255/255/255 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write

  printf  "${COLOR_RED_WD}sudo ./idll-test.exe --sram-read 1:100000000:3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read  ${COLOR_REST}\n"
  sudo ./idll-test.exe --sram-read 1:100000000:3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read

  printf  "${COLOR_RED_WD}sudo ./idll-test.exe --sram-read 2:1:3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read  ${COLOR_REST}\n"
  sudo ./idll-test.exe --sram-read 2:1:3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read

  printf  "${COLOR_RED_WD}sudo ./idll-test.exe --sram-read 5:1:3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read  ${COLOR_REST}\n"
  sudo ./idll-test.exe --sram-read 5:1:3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read
}
...
write(){
  ...
  print_command "sudo ./idll-test.exe --sram-write 1:0:$1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write"
  sudo ./idll-test.exe --sram-write 1:0:$1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write
}

read_data(){
  local n p data_length
  data_length=$(($1+1))
  launch_command "sudo ./idll-test.exe --sram-read 1:0:$data_length -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_read"
...
#
#      result=$(sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH $i --SRAM-DATA-FILE="./fakefile.txt" -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $m)
#      print_command "sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH $i --SRAM-DATA-FILE="./fakefile.txt" -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $m"
#      printcolor w "$result"
#      verify_result "$result"
#
#      if [[ "$status" == "fail" ]]; then
...
#    done
    ...
    while true; do
      ...
      result=$( sudo ./idll-test.exe --ADDRESS $((addresss-1)) --LENGTH $length --SRAM-DATA-FILE="./fakefile.txt" -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $m )
      print_command "sudo ./idll-test.exe --ADDRESS $((addresss-1)) --LENGTH $length --SRAM-DATA-FILE="./fakefile.txt" -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $m"
      printcolor w "$result"
      verify_result "$result"
  ...
  for k in "SramBankCopyManual" "SramAsyncBankCopyManual"; do
    ...
    for (( m = 0; m < bank-1; m++ )); do
      ...
      for (( i = 0; i < bank_capacity_hex; i=i+10000 )); do
        ...
        print_command "sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH 0x$i --SOURCE_BANK 0x$m --DEST_BANK 0x$((m+1)) -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $k"
        result=$(sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH 0x$i --SOURCE_BANK 0x$m --DEST_BANK 0x$((m+1)) -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $k)
...
bank_reset(){
  ...
  if [ "$bank" == "4" ]; then
    temp=$(sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH 0x$bank_capacity_hex --SOURCE_BANK 0x0 --DEST_BANK 0x1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramBankCopyManual)
    temp=$(sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH 0x$bank_capacity_hex --SOURCE_BANK 0x1 --DEST_BANK 0x2 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramBankCopyManual)
    temp=$(sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH 0x$bank_capacity_hex --SOURCE_BANK 0x2 --DEST_BANK 0x3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramBankCopyManual)
  else
    temp=$(sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH 0x$bank_capacity_hex --SOURCE_BANK 0x0 --DEST_BANK 0x1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramBankCopyManual)
  fi
}
...
#      for (( i = 0; i < bank_capacity_hex; i=i+10000 )); do
#        title b "Bank copy ($k) : Data length setting from small to bigger + all bank compare"
#        result=$(sudo ./idll-test.exe --SOURCE_BANK 0x$m --DEST_BANK 0x$((m+1)) --ADDRESS 0x$i --LENGTH 0x$((bank_capacity_hex-i)) -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $k)
#        print_command "sudo ./idll-test.exe --SOURCE_BANK 0x$m --DEST_BANK 0x$((m+1)) --ADDRESS 0x$i --LENGTH 0x$((bank_capacity_hex-i)) -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $k"
#        printcolor w "$result"
#        verify_result "$result"
#      done
#    done
#  done
  ...
  for h in "SramBankCompareManual" "SramAsyncBankCompareManual"; do
    ...
    for (( i = 0; i < bank_capacity_hex; i=i+steppingg )); do
      ...
      print_command "sudo ./idll-test.exe --sram-write 1:0x$i:$ran -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write"
      sudo ./idll-test.exe --sram-write 1:0x$i:"$ran" -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write


      launch_command "sudo ./idll-test.exe --SOURCE_BANK 0x0 --DEST_BANK 0x1 --ADDRESS 0x0 --LENGTH 0x$bank_capacity_hex -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $h"
      verify_result "$result"
...
#        read -p "enter key to continue..."
  ...
  sudo ./idll-test.exe --sram-write 1:0:1/2/3 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SRAM_Manual_write
  sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH 0x"$bank_capacity_hex" --SOURCE_BANK 0x0 --DEST_BANK 0x1 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramBankCopyManual
...
#===============================================================
crc32_caculate(){
#  idll-test --ADDRESS 0x0 --LENGTH 0x800 -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramAsyncCalculateCRC32Manual
  ...
  for l in "SramAsyncCalculateCRC32Manual" "SramCalculateCRC32Manual"; do
    for m in ${content_list[*]};do
      ...
      sudo ./idll-test.exe --ADDRESS $start_address --LENGTH $length --SRAM-DATA-FILE="temp.txt" -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramWriteWithVerifyManual

      title b "Test sram calculate CRC32 ($l)"
      title b "writing data : $m"
      launch_command "sudo ./idll-test.exe --ADDRESS $start_address --LENGTH $length -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $l"
  ...
  for l in "SramAsyncCalculateCRC32Manual" "SramCalculateCRC32Manual"; do
    ...
    launch_command "sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH $totalsize --SRAM-DATA-FILE="$file" -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramWriteWithVerifyManual"
#    print_command "sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH $totalsize --SRAM-DATA-FILE="fake8m.txt" -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramWriteWithVerifyManual"
#    sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH $totalsize --SRAM-DATA-FILE="fake8m.txt" -- --EBOARD_TYPE EBOARD_ADi_SA3X --section SramWriteWithVerifyManual
    launch_command "sudo ./idll-test.exe --ADDRESS 0x0 --LENGTH $totalsize -- --EBOARD_TYPE EBOARD_ADi_SA3X --section $l"
    compare_result "$result" "$crc"
  done
