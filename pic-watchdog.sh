...
SetWatchDogTimeOut() {
  ...
  read -p "Input the watchdog timeout, when you need the time is larger than 100 (second), or just enter to continue by default setting (0 10 100 200): " timeout
  ...
  if [[ "$timeout" -gt 100 ]]; then
    print_command "sudo ./idll-test --watchdog $timeout -- --EBOARD_TYPE EBOARD_ADi_BSEC_BACC --section PIC_Watchdog"
    sudo ./idll-test --watchdog $timeout -- --EBOARD_TYPE EBOARD_ADi_BSEC_BACC --section PIC_Watchdog
    ...
    for count in 0 10 100 200; do
      launch_command "sudo ./idll-test --watchdog $count -- --EBOARD_TYPE EBOARD_ADi_BSEC_BACC --section PIC_Watchdog"
...
WatchDogCallback() {
  ...
  if [[ "$timeout" -gt 100 ]]; then
    ...
    launch_command "sudo ./idll-test --WATCHDOG_TIMEOUT "$timeout" --SYSTEM_RESTART false -- --EBOARD_TYPE EBOARD_ADi_BSEC_BACC --section Callback_PIC_WDTimeout_Manual [CALLBACK][PIC][MANU]"
    compare_result "$result" "passed"

  else
    for i in 0 1 10 100; do
      title b "Test watchdog timeout callback timeout: $i sec"
      title b "Test watchdog timeout callback sytem restart: FALSE"
      launch_command "sudo ./idll-test --WATCHDOG_TIMEOUT $i --SYSTEM_RESTART false -- --EBOARD_TYPE EBOARD_ADi_BSEC_BACC --section Callback_PIC_WDTimeout_Manual [CALLBACK][PIC][MANU]"
...
loop_watchdog(){
  ...
  while true; do
    ...
    launch_command "sudo ./idll-test --WATCHDOG_TIMEOUT 1 --SYSTEM_RESTART false -- --EBOARD_TYPE EBOARD_ADi_BSEC_BACC --section Callback_PIC_WDTimeout_Manual [CALLBACK][PIC][MANU]"
...
WatchDog_RestartSystem() {
  ...
  if [[ "$timeout" -ne 0 ]]; then
    print_command "sudo ./idll-test --WATCHDOG_TIMEOUT $timeout --SYSTEM_RESTART true -- --EBOARD_TYPE EBOARD_ADi_BSEC_BACC --section Callback_PIC_WDTimeout_Manual [CALLBACK][PIC][MANU]"
    sudo ./idll-test --WATCHDOG_TIMEOUT $timeout --SYSTEM_RESTART true -- --EBOARD_TYPE EBOARD_ADi_BSEC_BACC --section Callback_PIC_WDTimeout_Manual [CALLBACK][PIC][MANU]
  fi
...
while true; do
  ...
  printf "${COLOR_RED_WD}3. WATCHDOG SYSTEM RESTART ${COLOR_REST}\n"
